...
const Header = () => {
  ...
  return <header className="main_header_area">
    ...
    <div className="header_menu" id="header_menu"><nav className="navbar navbar-default"><div className="container"><div className="navbar-flex d-flex align-items-center justify-content-between w-100">
      ...
      <div className={`navbar-collapse1 d-flex align-items-center ${isMenuOpen ? 'show' : ''}`}><ul className="nav navbar-nav" id="responsive-menu"><li className={`dropdown submenu ${isActive('/')}`}><Link to="/" className="dropdown-toggle">{t('common.home')}</Link></li><li className="dropdown submenu booking-catalog-dropdown"><button type="button" className="dropdown-toggle nav-bookings__toggle">BOOKING</button><div className="dropdown-menu booking-catalog-dropdown__menu"><BookingServiceMenu /></div></li><li className={`dropdown submenu ${isActive('/destinations')}`}><Link to="/destinations" className="dropdown-toggle">Top Destinations</Link></li><li className={`dropdown submenu ${isActive('/plan-my-day')}`}><Link to="/plan-my-day" className="dropdown-toggle">Plan My Day</Link></li><li className={`dropdown submenu ${isActive('/where-can-i-go')}`}><Link to="/where-can-i-go" className="dropdown-toggle">Where Can I Go?</Link></li><li className={`dropdown submenu ${isActive('/tours')}`}><Link to="/tours" className="dropdown-toggle">{t('common.tours')}</Link></li><li><a href="https://blog.optiontrip.com" target="_blank" rel="noopener noreferrer" className="dropdown-toggle">{t('common.blog')}</a></li><li className={isActive('/about')}><Link to="/about">About Us</Link></li><li className="search-main"><a href="#search1" className="mt_search" aria-label="Search"><i className="fa fa-search"></i></a></li></ul></div>
    ...
    <div className={`mobile-drawer ${isMenuOpen ? 'mobile-drawer--open' : ''}`}><div className="mobile-drawer__header"><Link to="/" onClick={toggleMenu}><img src="/images/newLogo.png" alt="OptionTrip" /></Link><button className="mobile-drawer__close" onClick={toggleMenu} aria-label="Close menu"><i className="fa fa-times"></i></button></div>
      <nav className="mobile-drawer__nav"><ul><li className={isActive('/')}><Link to="/" onClick={toggleMenu}>{t('common.home')}</Link></li><li className="mobile-drawer__booking"><span className="mobile-drawer__section-title">BOOKING</span><BookingServiceMenu mobile onNavigate={toggleMenu} /></li><li className={isActive('/destinations')}><Link to="/destinations" onClick={toggleMenu}>Top Destinations</Link></li><li className={isActive('/plan-my-day')}><Link to="/plan-my-day" onClick={toggleMenu}>Plan My Day</Link></li><li className={isActive('/where-can-i-go')}><Link to="/where-can-i-go" onClick={toggleMenu}>Where Can I Go?</Link></li><li className={isActive('/tours')}><Link to="/tours" onClick={toggleMenu}>{t('common.tours')}</Link></li><li><a href="https://blog.optiontrip.com" target="_blank" rel="noopener noreferrer" onClick={toggleMenu}>{t('common.blog')}</a></li><li className={isActive('/about')}><Link to="/about" onClick={toggleMenu}>About Us</Link></li><li className={isActive('/contact')}><Link to="/contact" onClick={toggleMenu}>{t('common.contact')}</Link></li></ul></nav>
